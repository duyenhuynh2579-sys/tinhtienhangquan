...
page = st.sidebar.selectbox("📋 Chọn trang", ["🍽️ Order", "🔑 Admin"])
...
if page == "🍽️ Order":
    ...
    with col1:
        ...
        # Sửa lỗi thụt lề nghiêm trọng ở phần nút bấm này
    ...
    # Thống kê hiển thị ngay dưới trang Order
    st.markdown("---")
    st.header("📊 Thống kê bán hàng")
    if st.session_state.history:
        history_df = pd.DataFrame(st.session_state.history)
        summary = (
            history_df.groupby("Tên món")["Số lượng"].sum().reset_index()
        )

        st.subheader("Tổng số lượng từng món đã bán")
        st.dataframe(summary, use_container_width=True)
        st.bar_chart(summary.set_index("Tên món"))
    else:
        st.info("Chưa có lịch sử bán hàng trong phiên này.")
...
# ==========================================
    ...
    # Tab phân chia thông tin cho gọn gàng
    tab1, tab2 = st.tabs(["📋 Danh sách thực đơn", "💰 Doanh thu & Lịch sử"])
    ...
    with tab2:
        if st.session_state.history:
            ...
            st.subheader("Chi tiết các hóa đơn đã thanh toán")
